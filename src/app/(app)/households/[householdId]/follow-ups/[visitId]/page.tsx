  ...
  return (
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title={t("overdue_visits")} value={overdue.length} icon={AlertCircle} color="red" isLoading={isLoading} />
        <StatCard title={t("due_this_month")} value={upcoming.length} icon={Clock} color="orange" isLoading={isLoading} />
        <StatCard title={t("visits_completed")} value={completed.length} icon={CalendarCheck} color="green" isLoading={isLoading} />
        <StatCard title={t("total_families")} value={totalFamilies} icon={Users} color="blue" isLoading={isLoading} />
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t("upcoming_this_month")} ({upcoming.length})
            </CardTitle>
          </CardHeader>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground space-y-2">
                ...
                <h3 className="font-semibold">{t("no_upcoming_month")}</h3>
              </div>
            )}
          </CardContent>
        </Card>
                ...
                <div key={visit.id} className="border p-4 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <p className="font-semibold">{householdName || t("unknown_family")}</p>
                      <Badge variant="secondary">{visit.visitType}</Badge>
                      <p className="text-sm text-muted-foreground">{t("by")} {visit.visitedBy}</p>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {new Date(visit.visitDate).toLocaleDateString()}
                    </p>
