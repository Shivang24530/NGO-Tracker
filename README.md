...
- **AI-Powered Insights (via Genkit):** Automatically analyzes child progress updates to identify trends.
...
This project is designed to be run in three parts simultaneously using three separate terminals.
...
Your AI features (like in `src/ai/flows`) run on a separate server.

```bash
npm run genkit:dev
```

This typically starts the Genkit server at http://localhost:3100.

Terminal 3: Start the Next.js Web App  
...
- `npm run genkit:dev` — Starts the Genkit AI server in development mode.  
